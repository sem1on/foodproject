...
function forms(formSelector, modalTimerId) {
    ...
    function showThanksModal(message) {
        const prevMadalDialog = document.querySelector('.modal__dialog');

        prevMadalDialog.classList.add('hide');
        ...
        setTimeout(() => {
            thanksModal.remove();
            prevMadalDialog.classList.add('show');
            prevMadalDialog.classList.remove('hide');
...
/* harmony export */   "default": () => (__WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
function tabs(tabsSelectot, tabsContentSelectot, tabsParentSelector, activeClass) {
    const tabs = document.querySelectorAll(tabsSelectot),
    tabsContent = document.querySelectorAll(tabsContentSelectot),
    ...
    tabsParent.addEventListener('click', function(event) {
        ...
        if(target && target.classList.contains(tabsSelectot.slice(1))) {
            tabs.forEach((item, i) => {
                if (target == item) {
                    hideTabContent();
                    showTabContent(i);
...
window.addEventListener('DOMContentLoaded', function() {
    ...
    (0,_modules_timer__WEBPACK_IMPORTED_MODULE_6__["default"])('.timer', '2022-01-00');
});    
})();
